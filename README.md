...
- **快进快退对齐主流电视播放器**：按一下跳 15 秒、连按累加成一次跳转，按住转为连续扫描且倍速递增（8x→16x→32x→64x），松手才真正 seek —— 长按只产生一次 seek，不会把 HLS 缓冲区反复冲掉
...
- **播放器内**：左右快进/快退（按一下 15 秒，按住连续扫描并加速），OK 暂停/继续，长按 OK 或菜单键切清晰度，上下呼出控制条/选集
