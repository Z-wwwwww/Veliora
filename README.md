...
本项目是一个 **Android TV 端的影视聚合搜索工具**：把聚合搜索与播放做成可以完全用遥控器操作的电视界面。大图沉浸式 UI，原生 ExoPlayer 播放器支持 4K 硬解直通，内置本地代理无需额外后端。
...
- **遥控器交互层**（`js/tv-remote.js`）：方向键焦点导航（几何最近邻算法，适配任意布局）、流媒体式主页（导航栏 + 顶部大图焦点跟随 + 横向内容排）、返回键两段式回顶
...
- **断点续播**：按集记忆播放进度，看完自动清除；上一集/下一集均适配遥控器
- **快进快退对齐主流电视播放器**：按一下跳 15 秒、连按累加成一次跳转，按住转为连续扫描且倍速逐档抬升（8x→…→按片长封顶，按住十几秒可从头扫到尾），松手才真正 seek —— 长按只产生一次 seek，不会把 HLS 缓冲区反复冲掉
- **快进时底部常驻进度条**：显示目标时间 / 总时长 / 偏移秒数 / 当前倍速，红条与浅色带之间就是「当前位置 ↔ 落点」，落点在整片里的位置一眼可见
...
- **播放器内**：左右快进/快退（按一下 15 秒，按住连续扫描并逐档加速，底部显示进度条与倍速），OK 暂停/继续，长按 OK 或菜单键切清晰度，上下呼出控制条/选集
