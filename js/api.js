        ...
        if (url.pathname === '/api/detail') {
            ...
            try {
                ...
                if (videoDetail.vod_play_url) {
                    ...
                    // 从单条线路中提取集数URL
                    const extractEpisodes = (source) => source.split('#').map(ep => {
                        const parts = ep.split('$');
                        // 返回URL部分(通常是第二部分，如果有的话)
                        return parts.length > 1 ? parts[1] : '';
                    }).filter(url => url && (url.startsWith('http://') || url.startsWith('https://')));

                    // 优先选择含 m3u8 直链的线路（部分源把网页播放器放第一条、m3u8放后面），
                    // 找不到则回退到第一条有效线路
                    let fallback = [];
                    for (const source of playSources) {
                        const eps = extractEpisodes(source);
                        if (eps.length === 0) continue;
                        if (eps.some(url => url.includes('.m3u8'))) {
                            episodes = eps;
                            break;
                        }
                        if (fallback.length === 0) fallback = eps;
                    }
                    if (episodes.length === 0) episodes = fallback;
                }
