...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - Veliora播放器';
...
function setupQualitySelector(hls) {
    ...
    if (levels.length <= 1) {
        // 单码率源无可选清晰度，移除可能残留的菜单（如切换源后）
        try { art.setting.remove('quality'); } catch (e) {}
        return;
    }
    ...
    const heightCount = {};
    levels.forEach(l => {
        if (l.height) heightCount[l.height] = (heightCount[l.height] || 0) + 1;
    });
    function levelLabel(level, index) {
        if (level.height) {
            return heightCount[level.height] > 1 && level.bitrate
    ...
    // 恢复上次手动选择的清晰度（按分辨率高度匹配）
    const saved = localStorage.getItem('preferredQuality') || 'auto';
    let savedIndex = -1;
    if (saved !== 'auto') {
        const savedHeight = parseInt(saved, 10);
        levels.forEach((l, i) => {
            if (l.height === savedHeight) savedIndex = i;
        });
    }
    if (savedIndex !== -1) {
    ...
    levels
        ...
        .forEach(({ level, index }) => {
            selector.push({
                html: levelLabel(level, index),
                value: index,
                default: index === savedIndex
            });
        });

    // update：已存在则整体替换，不存在则新增
    art.setting.update({
        name: 'quality',
        html: '清晰度',
        width: 220,
        tooltip: savedIndex === -1 ? '自动' : levelLabel(levels[savedIndex], savedIndex),
        selector: selector,
        onSelect: function (item) {
            hls.currentLevel = item.value;
            if (item.value === -1) {
                localStorage.setItem('preferredQuality', 'auto');
            } else if (levels[item.value] && levels[item.value].height) {
                localStorage.setItem('preferredQuality', String(levels[item.value].height));
            }
            return item.html;
        }
    });
...
function saveToHistory() {
    ...
    const pic_from_params = urlParams.get('pic') || ''; // 封面图，供历史记录展示
