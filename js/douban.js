    ...
    if (input) {
        ...
        try {
            ...
            window.history.pushState(
                { search: safeTitle }, 
                `搜索: ${safeTitle} - Veliora`, 
                `/s=${encodedQuery}`
            );
            // 更新页面标题
            document.title = `搜索: ${safeTitle} - Veliora`;
        } catch (e) {
            ...
            card.innerHTML = `
                <div class="relative w-full aspect-[2/3] overflow-hidden cursor-pointer" onclick="fillAndSearchWithDouban('${safeTitle}')">
                    <img src="${originalCoverUrl}" alt="${safeTitle}"
                        ...
                        onerror="if(window.nfImgFallback){nfImgFallback(this,'${originalCoverUrl}')}else{this.onerror=null;this.src='${proxiedCoverUrl}';this.classList.add('object-contain');}"
